...
const VaultsForm = ({ userID = "", setVaultForm, addVaultToList }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const folderToUpdate = existingFolders.find(
      (folder) => folder.name === vaultsData.folder_name
    );
      ...
      <div className=" fixed w-full md:w-1/2 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#F9F6F3] p-4 border-4 rounded-xl border-[#7A4FE7] shadow-lg z-50">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <input
            ...
            name="folder_name"
            value={vaultsData.folderName}
            className="border-2 border-gray-400 placeholder:font-medium hover:border-indigo-600 focus:outline-indigo-600 active:scale-[0.98] bg-transparent rounded-lg p-4"
          />
          <input
            onChange={handleChange}
            type="text"
          ...
          <input
            ...
            required
            value={vaultsData.confirmPassword}
            className="border-2 border-gray-400 placeholder:font-medium hover:border-indigo-600 focus:outline-indigo-600 active:scale-[0.98] bg-transparent rounded-lg p-4"
          />
          <div className="flex flex-wrap gap-4 justify-center">
            <button
              className="w-1/4 bg-[#7A4FE7] text-white p-2 rounded-lg"
              type="submit"
            >
              {isLoading ? (
...
VaultsForm.propTypes = {
  ...
  addVaultToList: PropTypes.func.isRequired,
};
